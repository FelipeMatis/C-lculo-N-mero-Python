...
# importa os módulos do seu projeto (verifique se os arquivos existem)
...
class App:
    ...
    def _build_ui(self):
        ...
        load_frame = ttk.LabelFrame(frm, text="Carregar Sistema (A e b)")
        load_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        load_frame.columnconfigure(1, weight=1)

        ttk.Button(load_frame, text="Carregar arquivo (A|b)", command=self.load_ab_file).grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(load_frame, text="Carregar A (apenas coeficientes)", command=self.load_a_file).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(load_frame, text="Carregar b (vetor)", command=self.load_b_file).grid(row=0, column=2, padx=5, pady=5)
        ...
        ttk.Label(load_frame, text="Colar A (linha por linha):").grid(row=2, column=0, sticky="w", padx=2, pady=(8,0))
        self.text_a = tk.Text(load_frame, height=5, width=50)
        self.text_a.grid(row=3, column=0, columnspan=2, padx=2, pady=2)

        ttk.Label(load_frame, text="Colar b (linha ou coluna):").grid(row=2, column=2, sticky="w", padx=2, pady=(8,0))
        self.text_b = tk.Text(load_frame, height=5, width=20)
        self.text_b.grid(row=3, column=2, padx=2, pady=2)

        ttk.Button(load_frame, text="Carregar do texto", command=self.load_from_text).grid(row=4, column=0, padx=5, pady=5)

        # === Parte: métodos lineares ===
        method_frame = ttk.LabelFrame(frm, text="Métodos Lineares")
        ...
        iter_frame = ttk.LabelFrame(frm, text="Parâmetros (métodos iterativos)")
        ...
        self.tol = tk.Entry(iter_frame, width=12); self.tol.insert(0, "1e-8"); self.tol.grid(row=0, column=1, padx=2, sticky="w")

        ttk.Label(iter_frame, text="Máx. iterações:").grid(row=0, column=2, sticky="w", padx=2)
        self.max_iter = tk.Entry(iter_frame, width=12); self.max_iter.insert(0, "1000"); self.max_iter.grid(row=0, column=3, padx=2, sticky="w")

        ttk.Label(iter_frame, text="Chute inicial x0 (separado por vírgula):").grid(row=1, column=0, columnspan=2, sticky="w", padx=2)
        self.x0_init = tk.Entry(iter_frame); self.x0_init.grid(row=1, column=2, columnspan=2, sticky="ew", padx=2)

        # === Parte: métodos para raízes ===
        roots_frame = ttk.LabelFrame(frm, text="Métodos para Raízes")
        ...
        self.root_a = tk.Entry(roots_frame, width=12); self.root_a.grid(row=1, column=1, sticky="w", padx=4)
        ttk.Label(roots_frame, text="b:").grid(row=1, column=2, sticky="e")
        self.root_b = tk.Entry(roots_frame, width=12); self.root_b.grid(row=1, column=3, sticky="w", padx=4)

        ttk.Label(roots_frame, text="x0:").grid(row=2, column=0, sticky="e")
        self.root_x0 = tk.Entry(roots_frame, width=12); self.root_x0.grid(row=2, column=1, sticky="w", padx=4)
        ttk.Label(roots_frame, text="x1:").grid(row=2, column=2, sticky="e")
        self.root_x1 = tk.Entry(roots_frame, width=12); self.root_x1.grid(row=2, column=3, sticky="w", padx=4)

        ttk.Label(roots_frame, text="Tolerância:").grid(row=3, column=0, sticky="e")
        self.root_tol = tk.Entry(roots_frame, width=12); self.root_tol.insert(0, "1e-6"); self.root_tol.grid(row=3, column=1, sticky="w", padx=4)
        ttk.Label(roots_frame, text="Max iterações:").grid(row=3, column=2, sticky="e")
        self.root_maxiter = tk.Entry(roots_frame, width=12); self.root_maxiter.insert(0, "100"); self.root_maxiter.grid(row=3, column=3, sticky="w", padx=4)

        # opção para ver passos dos métodos de raízes
        self.var_show_roots_steps = tk.BooleanVar(value=False)
        ttk.Checkbutton(roots_frame, text="Mostrar passos", variable=self.var_show_roots_steps).grid(row=4, column=0, columnspan=2, sticky="w", padx=4)

        # botões de carregar entrada opcional e executar método de raízes
        ttk.Button(roots_frame, text="Carregar entrada.txt (opcional)", command=self.load_entrada_file).grid(row=5, column=0, padx=4, pady=4)
        ttk.Button(roots_frame, text="Resolver Raiz", command=self.run_roots).grid(row=5, column=1, padx=4, pady=4)

        # === Botões principais ===
        btn_frame = ttk.Frame(frm); btn_frame.grid(row=4, column=0, sticky="ew", pady=5)
        ttk.Button(btn_frame, text="Resolver Método Linear", command=self.resolver_linear).grid(row=0, column=0, padx=5)
        ttk.Button(btn_frame, text="Limpar", command=self.limpar).grid(row=0, column=1, padx=5)
        ttk.Button(btn_frame, text="Salvar Resultado (.txt)", command=self.salvar_resultado).grid(row=0, column=2, padx=5)

        # === Área de resultados (abas) ===
        result_nb = ttk.Notebook(frm); result_nb.grid(row=5, column=0, sticky="nsew", padx=5, pady=5)
        frm.rowconfigure(5, weight=1)

        # aba: resultados resumidos
        result_frame = ttk.Frame(result_nb); result_nb.add(result_frame, text="Resultados")
        result_frame.columnconfigure(0, weight=1); result_frame.rowconfigure(0, weight=1)
        self.texto_resultado = tk.Text(result_frame, height=20, wrap=tk.NONE); self.texto_resultado.grid(row=0, column=0, sticky="nsew")
        vsb = ttk.Scrollbar(result_frame, orient="vertical", command=self.texto_resultado.yview); vsb.grid(row=0, column=1, sticky="ns")
        self.texto_resultado.configure(yscrollcommand=vsb.set)
        hsb = ttk.Scrollbar(result_frame, orient="horizontal", command=self.texto_resultado.xview); hsb.grid(row=1, column=0, sticky="ew")
        self.texto_resultado.configure(xscrollcommand=hsb.set)

        # aba: passos detalhados
        steps_frame = ttk.Frame(result_nb); result_nb.add(steps_frame, text="Passos / Saída detalhada")
        steps_frame.columnconfigure(0, weight=1); steps_frame.rowconfigure(0, weight=1)
        self.texto_passos = tk.Text(steps_frame, height=20, wrap=tk.NONE); self.texto_passos.grid(row=0, column=0, sticky="nsew")
        vsb2 = ttk.Scrollbar(steps_frame, orient="vertical", command=self.texto_passos.yview); vsb2.grid(row=0, column=1, sticky="ns")
        self.texto_passos.configure(yscrollcommand=vsb2.set)
        hsb2 = ttk.Scrollbar(steps_frame, orient="horizontal", command=self.texto_passos.xview); hsb2.grid(row=1, column=0, sticky="ew")
    ...
    def _build_linear_options(self):
        ...
        for w in self.options_frame.winfo_children():
            w.destroy()
        ttk.Label(self.options_frame, text="Opções específicas do método:").grid(row=0, column=0, sticky="w")
        self.var_show_steps = tk.BooleanVar(value=False)
        ttk.Checkbutton(self.options_frame, text="Mostrar passos detalhados", variable=self.var_show_steps).grid(row=1, column=0, sticky="w", padx=4)
        self.var_show_matrices = tk.BooleanVar(value=False)
        ttk.Checkbutton(self.options_frame, text="Incluir matrizes em cada passo", variable=self.var_show_matrices).grid(row=2, column=0, sticky="w", padx=4)
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                ...
                if len(tokens) < 7:
                    ...
                    return
                metodo = int(tokens[0])
                a = tokens[1]; b = tokens[2]; x0 = tokens[3]; x1 = tokens[4]; tol = tokens[5]; maxit = tokens[6]

                self.root_method_var.set(["Bisseção","Ponto Fixo","Newton-Raphson","Secante","Regula Falsi"][metodo-1])
                self.root_a.delete(0, tk.END); self.root_a.insert(0, a)
                self.root_b.delete(0, tk.END); self.root_b.insert(0, b)
                self.root_x0.delete(0, tk.END); self.root_x0.insert(0, x0)
                self.root_x1.delete(0, tk.END); self.root_x1.insert(0, x1)
                self.root_tol.delete(0, tk.END); self.root_tol.insert(0, tol)
                self.root_maxiter.delete(0, tk.END); self.root_maxiter.insert(0, maxit)
    ...
    def resolver_linear(self):
        ...
        try:
            ...
            if "iterativo" in metodo_nome.lower():
                try:
                    tol = float(self.tol.get()); max_iter = int(self.max_iter.get())
                except Exception:
    ...
    def run_roots(self):
        ...
        dados = MR.DadosEntrada(1, a, b, x0, x1, tol, maxit)  # método será ajustado abaixo
        mapping = {
    ...
    def limpar(self):
        ...
        self.texto_resultado.delete('1.0', tk.END)
        self.texto_passos.delete('1.0', tk.END)
        self.text_a.delete("1.0", tk.END); self.text_b.delete("1.0", tk.END)
        self.x0_init.delete(0, tk.END); self.x0_init.insert(0, "")
        self.root_a.delete(0, tk.END); self.root_b.delete(0, tk.END)
        self.root_x0.delete(0, tk.END); self.root_x1.delete(0, tk.END)
        self.root_tol.delete(0, tk.END); self.root_tol.insert(0, "1e-6")
        self.root_maxiter.delete(0, tk.END); self.root_maxiter.insert(0, "100")
        if hasattr(self, "var_show_steps"): self.var_show_steps.set(False)
        if hasattr(self, "var_show_matrices"): self.var_show_matrices.set(False)
        if hasattr(self, "var_show_LU"): self.var_show_LU.set(False)
        if hasattr(self, "var_show_permutation"): self.var_show_permutation.set(False)
        if hasattr(self, "var_show_roots_steps"): self.var_show_roots_steps.set(False)
