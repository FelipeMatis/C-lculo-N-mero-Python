# interface_gui.py
# GUI em Tkinter com integração dos métodos lineares e dos métodos para raízes (bissecao, newton...)
# Coloque este arquivo na mesma pasta de metodos_lineares.py e metodos_raizes.py
...
# módulos do seu projeto (certifique-se de que metodos_lineares.py e metodos_raizes.py estão na pasta)
from metodos_lineares import METODOS as METODOS_LIN
import metodos_raizes as MR   # arquivo com Bisseção, Newton, Secante, etc.
...
class App:
    ...
    def _build_ui(self):
        ...
        # ---------------------------
        # Carregamento / entradas A e b
        # ---------------------------
        load_frame = ttk.LabelFrame(frm, text="Carregar Sistema (A e b)")
        load_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        load_frame.columnconfigure(1, weight=1)

        btn_load_ab = ttk.Button(load_frame, text="Carregar arquivo (A|b)", command=self.load_ab_file)
        btn_load_ab.grid(row=0, column=0, padx=5, pady=5)

        btn_load_a = ttk.Button(load_frame, text="Carregar A (só coeficientes)", command=self.load_a_file)
        btn_load_a.grid(row=0, column=1, padx=5, pady=5)

        btn_load_b = ttk.Button(load_frame, text="Carregar b (vetor)", command=self.load_b_file)
        btn_load_b.grid(row=0, column=2, padx=5, pady=5)
        ...
        # Área para colar A e b diretamente (sem arquivos)
        ...
        ttk.Label(load_frame, text="Colar b (uma linha ou coluna):").grid(row=2, column=2, sticky="w", padx=2, pady=(8,0))
        self.text_b = tk.Text(load_frame, height=5, width=20)
        self.text_b.grid(row=3, column=2, padx=2, pady=2)

        btn_from_text = ttk.Button(load_frame, text="Carregar do Texto", command=self.load_from_text)
        btn_from_text.grid(row=4, column=0, padx=5, pady=5)

        # ---------------------------
        # Métodos lineares (existentes)
        # ---------------------------
        ...
        # Parâmetros iterativos gerais (aparecem sempre, usados somente se aplicável)
        ...
        ttk.Label(iter_frame, text="Max iterações:").grid(row=0, column=2, sticky="w", padx=2)
        ...
        # Campos para a, b, x0, x1, tol, maxIter
        ...
        ttk.Label(roots_frame, text="maxIter:").grid(row=3, column=2, sticky="e")
        ...
        # ---------------------------
        # Botões principais / salvar
        # ---------------------------
        btn_frame = ttk.Frame(frm)
        btn_frame.grid(row=4, column=0, sticky="ew", pady=5)
        btn_resolver = ttk.Button(btn_frame, text="Resolver Método Linear", command=self.resolver_linear)
        btn_resolver.grid(row=0, column=0, padx=5)
        btn_limpar = ttk.Button(btn_frame, text="Limpar", command=self.limpar)
        btn_limpar.grid(row=0, column=1, padx=5)
        btn_salvar = ttk.Button(btn_frame, text="Salvar Resultado (.txt)", command=self.salvar_resultado)
        btn_salvar.grid(row=0, column=2, padx=5)

        # ---------------------------
        # Área de resultados e passos (abas)
        # ---------------------------
        result_nb = ttk.Notebook(frm)
        result_nb.grid(row=5, column=0, sticky="nsew", padx=5, pady=5)
        frm.rowconfigure(5, weight=1)

        # Aba resultados
        result_frame = ttk.Frame(result_nb)
        result_nb.add(result_frame, text="Resultados")
        ...
        steps_frame = ttk.Frame(result_nb)
        result_nb.add(steps_frame, text="Passos / Saída detalhada")
    ...
    def _build_linear_options(self):
        ...
        cb1 = ttk.Checkbutton(self.options_frame, text="Mostrar passos detalhados", variable=self.var_show_steps)
        cb1.grid(row=1, column=0, sticky="w", padx=4)
        self.var_show_matrices = tk.BooleanVar(value=False)
        cb2 = ttk.Checkbutton(self.options_frame, text="Incluir matrizes em cada passo", variable=self.var_show_matrices)
        cb2.grid(row=2, column=0, sticky="w", padx=4)
        ...
        self.cb_perm = ttk.Checkbutton(self.options_frame, text="Exibir permutações (pivoteamento completo)", variable=self.var_show_permutation)
    ...
    # -------------------------
    # Carregamento de arquivos A|b, A, b
    # -------------------------
    ...
    def load_from_text(self):
        ...
        try:
            ...
            # Atualizar status e informar usuário
    ...
    # -------------------------
    # Carregar entrada.txt (para métodos de raízes) - opcional
    # -------------------------
    def load_entrada_file(self):
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                # assume os 7 valores na primeira linha (ou separados)
                ...
                # preencher campos da UI (adaptando os tipos)
    ...
    # -------------------------
    # Run - Métodos Lineares (chama METODOS_LIN)
    # -------------------------
    def resolver_linear(self):
        ...
        show_steps = self.var_show_steps.get()
        show_matrices = self.var_show_matrices.get()
        show_LU = self.var_show_LU.get()
        show_perm = self.var_show_permutation.get()
        try:
                ...
                if x0_str == "":
                    ...
                    try:
                        ...
                        if len(x0_parts) != self.b.shape[0]:
                            x0 = np.zeros(self.b.shape[0]); L=min(len(x0_parts),len(x0)); x0[:L]=x0_parts[:L]
                        else:
        ...
        try:
            if x is not None:
                ...
                self.texto_resultado.insert(tk.END, f"Residuo ||Ax - b||_2 = {residuo:.6e}\n")
        except:
            pass

        # exibir passos se houver
        if steps:
    ...
    # -------------------------
    # Run - Métodos de Raízes (integração)
    # -------------------------
    def run_roots(self):
        # limpa
        self.texto_resultado.delete('1.0', tk.END)
        ...
        # se usuário preferiu carregar entrada.txt, ele já está preenchido nos campos por load_entrada_file
        ...
        # montar estrutura compatível com metodos_raizes.DadosEntrada
        dados = MR.DadosEntrada(1, a, b, x0, x1, tol, maxit)  # metodo será substituído
        # escolher função
        mapping = {
        ...
        # ajustar o campo metodo no objeto
        dados.metodo = metodo_id

        # We'll capture the text output in a StringIO and pass it as 'saida' to functions
        saida_io = io.StringIO()
        try:
            func(dados, saida_io)  # chama a função, que escreve em saida_io
        except Exception as e:
        ...
        if self.var_show_roots_steps.get():
            # envia tudo também para a aba passos
            self.texto_passos.insert(tk.END, texto)

        # opcional: também salvar em resultado.txt no disco
        try:
    ...
    def limpar(self):
        ...
        self.var_show_steps.set(False); self.var_show_matrices.set(False)
        self.var_show_LU.set(False); self.var_show_permutation.set(False)
